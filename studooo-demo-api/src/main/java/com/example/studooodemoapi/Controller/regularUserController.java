...
public class regularUserController {
    ...
    public ResponseEntity<regularUser> changePass(@PathVariable String username, @RequestBody regularUser user) {
        return new ResponseEntity<>(regularUserService.changeInfo(username, user), HttpStatus.CREATED);
    }

    @GetMapping("/filterSearch/{offset}/{pageSize}")
    public ResponseEntity<List<regularUser>> getRegularUsersByFilter(@PathVariable int offset, @PathVariable int pageSize, @RequestParam String letter) {
        return new ResponseEntity<>(regularUserService.getRegularUsersByFilter(offset, pageSize, letter).getContent(), HttpStatus.OK);
    }
